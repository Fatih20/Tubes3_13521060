...
class Main {
    ...
    getMatchingQuestion(substring: string, flag: boolean): string[] {
        ...
        this.ListQuestion.forEach(element => {
            ...
            if (exactMatchExist) { retval = [element]; return [element]; }
            else {
        ...
        if (elmt != undefined && elmt[0] > 0.9) {
            ...
            retval = [elmt[1], Questions.queue[0][1] == undefined? "": Questions.queue[0][1], Questions.queue[1][1] == undefined? "": Questions.queue[1][1]]
